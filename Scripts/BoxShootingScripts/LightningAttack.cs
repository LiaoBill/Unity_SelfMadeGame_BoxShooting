...
public class LightningAttack : MonoBehaviour {
    ...
    void Start () {
        cool_down_time = GameManager.getInstance().GetComponent<BoxSkillDataManager>().GetCoolDown(0);
        cool_down_times = -1;
    }
    int cool_down_times;
    ...
    IEnumerator AttackWithLightning()
    {
        is_attacking = true;
        //set bolt
        GameObject bolt = PoolManager.getInstance().GetObject(2);
        ...
        if (Physics.Raycast(ray, out hit, Mathf.Infinity, layerMask))
        {
            ...
            GameObject attackPoint = PoolManager.getInstance().GetObject(1);
	...
	void Update () {
        ...
        {
            if (AttackManager.getInstance().GetAttackStatus() == 1)
            {
                if (!is_attacking)
